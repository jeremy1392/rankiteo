...
if __name__ == "__main__":
    ...
    for breach_type in breach_types:
        ...
        m1_prob = predict_probabilities('model1.pkl', input_data_rf)
        m2_prob = predict_probabilities('model2.pkl', input_data_lr)

        # Combine the probabilities (average)
        combined_prob = (m1_prob[0] + m2_prob[0]) / 2

        # Predict severity using the GLM model with intervals, quantiles, and Expected Shortfall
        severity, prediction_interval_95, quantile_99, es_95, es_99, quantile_95 = predict_glm_severity_with_intervals('model3.pkl', input_data_glm)
